...
(function() {
  ...
  /*
   * The SQL database, only available if m_built.
   */
  var m_db;
  
  /*
   * The number of records that have been added to the main charlist and
   * wordlist tables.
   * 
   * This is used for explicitly generating integer primary keys for
   * records in those tables, so that the generated key can then be used
   * in foreign key statements.  For the other tables, we don't care
   * about the primary key values and can just leave those
   * autogenerated.
   */
  var m_count_charlist;
  var m_count_wordlist;
  ...
  /*
   * Add a character record to the SQL database.
   * 
   * You must call initDB() before using this function.
   * 
  ...
  function addCRecord(ce) {
    ...
    // Increase the charlist count so we get a unique primary key value
    m_count_charlist++;
    
    // Insert an entry into the charlist table
    if ("dfn" in ce) {
      // Definition present
      m_db.run(
        "INSERT INTO charlist (cid, cpv, cgl) VALUES (?, ?, ?)",
        [m_count_charlist, ce.cpv, ce.dfn]
      );
      
    } else {
      // Definition not present
      m_db.run(
        "INSERT INTO charlist (cid, cpv) VALUES (?, ?)",
        [m_count_charlist, ce.cpv]
      );
    }
    
    // Insert any Jyutping readings into the charjyu table and link them
    // to the record we just inserted in charlist
    for(i = 0; i < ce.crd.length; i++) {
      m_db.run(
        "INSERT INTO charjyu (cid, jor, jyu) VALUES (?, ?, ?)",
        [m_count_charlist, i + 1, ce.crd[i]]
      );
    }
  }
  
  /*
   * Add a word record to the SQL database.
   * 
   * You must call initDB() before using this function.
   * 
  ...
  /*
   * Initialize the SQL database so that you may begin using this
   * module.
   * 
   * SQL is the SQL object that is generated by initSqlJs() of the
   * sql.js library.
   * 
   ...
   * Parameters:
   * 
   *   SQL : SqlJs - the sql.js main object
   */
  function initDB(SQL) {
    ...
    // Define the database tables and indices
    m_db.run(
  ...
  window.ctt_main = {
    ...
    "charQuery": charQuery,
    "seekCode": seekCode,
    "addCRecord": addCRecord,
    "addWRecord": addWRecord,
    "initDB": initDB
  };  
